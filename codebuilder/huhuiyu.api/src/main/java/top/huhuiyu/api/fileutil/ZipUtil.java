...
public class ZipUtil {
   ...
   *
   * @param dir  要添加的目录
   * @param root 目录在zip中的路径
   * @param zos  zip输出流
   * 
   * @throws Exception 处理发生异常
   */
  private void makeZip(File dir, String root, ZipOutputStream zos) throws Exception {
    if (!root.endsWith(split) && (root.length() > 1)) {
      root = root + split;
    }
    // 创建目录自己到zip
   ...
   *
   * @param file 要添加的文件
   * @param root 要添加的文件在zip中的路径
   * @param zos  zip輸出流 chang
   * 
   * @throws Exception 处理发生异常
   */
  private void makeFileZip(File file, String root, ZipOutputStream zos) throws Exception {
    // 確保root路徑格式是正確
    if (!root.endsWith(split) && (root.length() > 1)) {
      root = root + split;
    }
  ...
  public static void unzip(String zipFile, String outputDir) throws Exception {
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (e.isDirectory()) {
        ...
        FileOutputStream fos = new FileOutputStream(new File(output, "/" + e.getName()));
        byte[] bytes = new byte[FileUtil.DEFAULT_READ_SIZE];
